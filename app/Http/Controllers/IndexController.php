...
class IndexController extends Controller
{
    ...
    private function filter($players, $sortBy = "meetCount", $sort = "desc", $mode = 'all', $number = 50)
    {
        ...
        switch ($sortBy) {
            ...
            case ("timeMet"):
                usort($players, function ($a, $b) use ($sort) {
                    if ($sort == "desc") {
                        return $b['timeMet'] <=> $a['timeMet'];
                    } else {
                        return $a['timeMet'] <=> $b['timeMet'];
                    }
                });
                break;
        }
    ...
    {
        /* get file from ressources/json */
        $json = file_get_contents(resource_path('json/player_counter.json'));
        /* decode json */
        ...
        $data = new \stdClass();
        $data->lastUpdated = date('H:i:s d.m.Y', filemtime(resource_path('json/player_counter.json')));
        $data->players = $players;

        return $data;
    }

    private function getAllGameMode()
    {
        $gameMode = array();
        $gameMode['Duel'] = "Duel";
